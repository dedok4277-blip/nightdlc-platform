...
async function testKeySystem() {
  ...
  try {
    ...
    // Получаем админа
    const [admins] = await connection.execute('SELECT id, username FROM users WHERE is_admin = 1 LIMIT 1')
    ...
    const [users] = await connection.execute('SELECT id, username FROM users WHERE is_admin = 0 LIMIT 1')
    ...
    if (users.length > 0) {
      ...
      // Проверяем ключ
      const [keyCheck] = await connection.execute('SELECT * FROM license_keys WHERE `key` = ? AND used = 0', [basicKey.key])
      ...
      if (keyCheck.length > 0) {
        ...
        await connection.execute('UPDATE license_keys SET used = 1, used_by = ?, used_at = ? WHERE `key` = ?', [testUser.id, Date.now(), basicKey.key])
        await connection.execute('UPDATE users SET license_key = ?, subscription_tier = ?, subscription_expires_at = ? WHERE id = ?', [basicKey.key, 'Basic', expiresAt, testUser.id])
        
        console.log('   ✅ Ключ активирован!')
        
        // Проверяем результат
        const [updatedUser] = await connection.execute('SELECT subscription_tier, subscription_expires_at FROM users WHERE id = ?', [testUser.id])
        const [updatedKey] = await connection.execute('SELECT used, used_by FROM license_keys WHERE `key` = ?', [basicKey.key])
        ...
        const [keyCheck2] = await connection.execute('SELECT * FROM license_keys WHERE `key` = ? AND used = 0', [basicKey.key])
